# 파이썬 웹프로그램 : django(프레임워크), flask, fastAPI(마이크로 프레임워크)
# 1. fastAPI : 비동기 restAPI 서버(웹) 프로그램을 만드는 데 특화
    # HTTP방식 : GET(read), POST(create), PUT(update), DELETE(delete)
    # 비동기 방식에서 쓰이는 파라미터 전달 방식 : /empsch/scott > /empsch?ename=scott
# 2. 자동 데이터 검증 및 문서화
...
@app.get('/')
async def index(): # async : 비동기 처리(안 쓰면 동기함수)
